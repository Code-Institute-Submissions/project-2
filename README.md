...
The Destination Interest Finder website calls upon the Google Maps API and the Google Places API to allow users reseach their next travel destination. 

The user is able to selection a location and then find a point of interest at that location.
...
Features
- Country selection feature: A user wants to find the location of a country on a map. => A user selects a country from the country drop down box => The map zooms to that location.
...
- Show information about selected point of interest feature: A user wants to find out more information on a point of interest returned in the search results => A user selects the icon on the map or the name of the point of interest in the results table => More information about the selected point of interest is displayed on the map.
- Clear all results feature: A user want to clear all the results and start a new search => The user clicks on the "clear" button => The maps, results table and all the texts fields are returned to their initial state.

### Features Left to Implement
- In the future the application may be updated to include the functionality to select additional points of interest.

## Technologies Used

The technologies used within this project and the reason for their use are listed below. 
...
- [JQuery](https://jquery.com)
    - The project uses **JQuery** to read a data file
- [Google APIs](https://jquery.com)
    - The project uses **Google Maps API and Google Places API** to auto-complete and locate cities on a map and to search for points of interest in those cities. 
...
- The User selects a country from the country drop down box.
...
- Page opens - The country, city and radio buttons are blank. The map is zoomed out to view the world.
- The User leaves the country drop down box set to all and starts to type a city, the city name auto completes and the user selects the city.
...
- On a desktop: the logo and header text are displayed side by side -> The selection area and map are displayed underneath side by side. -> The results table is displayed underneath the map
- On a phone/tablet: the logo and header text are displayed underneath each other -> The selection area and map are displayed underneath each other -> The results table is displayed underneath the map
...
- Markers are dropped onto the map, starting at A and continuing depending on how many points of interest there are.
...
- User either selects the point of interest marker on the map or points of interest from the results table.
- Information about that point of interest is displayed beside the marker on the map. The information contains Name, address, Telephone number, Rating and Website.
- On a desktop: the logo and header text are displayed side by side -> The selection area and map are displayed underneath side by side. -> The results table is displayed underneath the map
- On a phone/tablet: the logo and header text are displayed underneath each other -> The selection area and map are displayed underneath each other -> The results table is displayed underneath the map
...
- The user selects a country and types a city -> clicks the "Clear" button, the page resets.
- The user selects a country and types a city, selects the "Attractions" radio button -> clicks the "Clear" button, the page resets.
- The user selects a country and types a city, selects the "Accomodation" radio button -> clicks the "Clear" button, the page resets.
- The user selects a country and types a city, selects the "Bar and Restaurants" radio button -> clicks the "Clear" button, the page resets.

During testing
- Adjustments had to be made to ensure content displayed correctly on different screen sizes
- The map wasn't adjusting correctly when a country was selected
- The radio buttons could of been selected without selecting a city first.
